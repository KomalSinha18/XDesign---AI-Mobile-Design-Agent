...
import { z } from "zod";
import { openaiClient } from "@/lib/openrouter";
  ...
  async ({ event, step, publish }) => {
    ...
    const analysis = await step.run("analyze-and-plan-screens", async () => {
      ...
      try {
        ...
        console.error("Error in analysis step - Full error:", {
        ...
        if (
          ...
          throw new Error(
            `OpenAI API authentication failed: ${errorDetails}. Please check your OPENAI_API_KEY is valid.`
          );
        }
        ...
        throw new Error(
          `Failed to analyze project: ${errorDetails}${error?.statusCode ? ` (Status: ${error.statusCode})` : ""}`
        );
      }
    ...
    for (let i = 0; i < analysis.screens.length; i++) {
      ...
      `.trim(),
          ...
          // Log full error details for debugging
          console.error(`Error generating screen ${i + 1} - Full error:`, {
            message: error?.message,
            name: error?.name,
            cause: error?.cause,
            providerError: error?.providerError,
          ...
          // Re-throw with more context
          throw new Error(
            `Failed to generate screen "${screenPlan.name}": ${errorDetails}${error?.statusCode ? ` (Status: ${error.statusCode})` : ""}`
          );
        }
